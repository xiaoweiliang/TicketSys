 ...
 * @return mixed
 */
function S($name,$value='',$options=null,$model='') {
    ...
    if(is_array($options)){
        ...
        $cache      =   Think\Cache::getInstance($model);
    }
    if(''=== $value){ // 获取缓存
        return $cache->get($name,$model);
    }elseif(is_null($value)) { // 删除缓存
        return $cache->rm($name,$model);
    }else { // 缓存数据
        if(is_array($options)) {
        ...
        return $cache->set($name, $value, $expire,$model);
    }
}
/**
 * 清除缓存
 */
function RM($name,$model=null)
{
        static $cache   =   '';
        $cache      =   Think\Cache::getInstance($model);
        return $cache->rm($name,$model);
