...
class Think {
    ...
    static public function start() {
      ...
      spl_autoload_register('Think\Think::autoload');                                         //当下面系统在执行过程中如果发现类没有加载   会自动调用autoload 加载对应的class 
      // 设定错误和异常处理
    ...
    public static function autoload($class) {
        // 检查是否存在映射
       // ToolBack::_ECHO($class);
        if(isset(self::$_map[$class])) {
            include self::$_map[$class];
        }elseif(false !== strpos($class,'\\')){
          $name           =   strstr($class, '\\', true);                          //获取字符串中第一个\\左边的字符串
          if(in_array($name,array('Think','Org','Behavior','Com','Vendor')) || is_dir(LIB_PATH.$name)){ 
              // Library目录下面的命名空间自动定位
              //ToolBack::_ECHO("2".LIB_PATH);
              $path       =   LIB_PATH;
          }else{
            ...
            //  ToolBack::_ECHO("21".$path);
          }
         // ToolBack::_ECHO("3".$path);
          $filename       =   $path . str_replace('\\', '/', $class) . EXT;
          //ToolBack::_ECHO("4".$filename);
          if(is_file($filename)) {
              // Win环境下面严格区分大小写
