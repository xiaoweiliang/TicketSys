...
class Http {
    ...
    */
    static public function fsockopenDownload($url, $conf = array()) {
        ...
        /*
         * 这里的${$K}      (可变变量)
         * 意思是   假如$k='hello'    $hello='world'                那么${$K}就是world         
         */
        
        if($post) {                       //$post来自上面的foreach中    ${$k}        原来的数组中有一个参数是post  此时$K是post     那么${$k} 就是$post
            if(is_array($post))
            {
                $post = http_build_query($post);                   //生成请求参数
                /*  <?php
        ...
        $fp = @fsockopen(($ip ? $ip : $host), $port, $errno, $errstr, $timeout);                   //返回远程文件操作句柄
        if(!$fp) {
            return '';
        } else {
